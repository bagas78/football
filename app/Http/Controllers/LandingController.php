...
class LandingController extends Controller
{
    ...
    public function hasil(Request $request){
        ...
        if (@$musim) {
            ...
            $data['hasil_data'] = DB::select("SELECT * FROM jadwal AS a JOIN musim AS b ON a.jadwal_musim = b.musim_id WHERE a.jadwal_status = 1 AND a.jadwal_musim = $musim AND a.jadwal_delete = 0 ORDER BY a.jadwal_pekan + 0 ASC");

        } else {

            $data['hasil_data'] = DB::select("SELECT * FROM jadwal AS a JOIN musim AS b ON a.jadwal_musim = b.musim_id WHERE a.jadwal_status = 1 AND a.jadwal_delete = 0 ORDER BY a.jadwal_pekan + 0 ASC");
        }
    ...
    public function histori_view($id = '',Request $request){
        ...
        return view('landing/histori_view',$data);
    }
}
